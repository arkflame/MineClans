...
public class RelationsDAO {
    ...
    public void createTable() {
        mySQLProvider.executeUpdateQuery("CREATE TABLE IF NOT EXISTS relations (" +
    ...
    public void insertRelation(UUID factionId, UUID targetFactionId, String relationType) {
        mySQLProvider.executeUpdateQuery("INSERT INTO relations (faction_id, target_faction_id, relation_type) VALUES (?, ?, ?)",
    ...
    public void insertOrUpdateRelation(UUID factionId, UUID targetFactionId, String relationType) {
        String query = "INSERT INTO relations (faction_id, target_faction_id, relation_type) " +
                       "VALUES (?, ?, ?) " +
    ...
    public void removeRelationById(UUID factionId, UUID targetFactionId) {
        mySQLProvider.executeUpdateQuery("DELETE FROM relations WHERE faction_id = ? AND target_faction_id = ?",
    ...
    public void removeRelationsById(UUID id) {
        ...
        mySQLProvider.executeUpdateQuery("DELETE FROM relations WHERE faction_id = ?", id);
    
        // Remove relations where the target_faction_id matches the given ID
        mySQLProvider.executeUpdateQuery("DELETE FROM relations WHERE target_faction_id = ?", id);
    }
